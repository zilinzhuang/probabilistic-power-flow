...
MODEL = data33origin; % 负荷、支路阻抗数据均为 标幺值
...
ps1=solar2power(sunLHS,700)*1000;       %  太阳能功率
...
% PQ(V)型 DG
PQVNUM = [];    % PQV型DG位置  定速型异步风机（转差率不变，导致电压不变、无功吸收不变）
Ppqv_G = [30/SB]; % kW >>> p.u.
s_pqv=0.3;       % 转差率
...
% PQ型 DG        双馈风机、同步全功率风机
PQNUM = [33]; % PQ型DG位置          
Ppq_G = [pw1(1,mci)/SB , pw1(1,mci)/SB,pw1(1,mci)/SB]; % kW >>> p.u.
Qpq_G = Ppq_G.*tan(acos(0.98)); % kVar >>> p.u  %假定功率因数恒定

% PI型 DG
PINUM = [8];    % PI型DG位置、光伏（电流型逆变器）        
Ppi_G = [50/SB]; % kW >>> p.u.
Ipi_G = [-10/IB]; % A >>> p.u
Q_PI=zeros(1,length(PINUM));
Q_OLDPI=zeros(1,length(PINUM));
...
% PV型 DG         燃料电池、光伏（电压型逆变器）
PVNUM = [18]; % PV型DG位置
Ppv_G = [ps1(1,mci)/SB/2,ps1(1,mci)/SB/2]; % kW >>> p.u
Qpv_G = [200/SB,200/SB]; % PV型DG的无功出力调节范围，这里仍按照恒功率模型计算。当无功大于此值，转为PQ节点。kVar>>> p.u
...
figure(1)
legend('不加入分布式电源','接入33节点','接入18、33节点','接入8、15、18、33节点')
figure(2)
legend('不加入分布式电源','接入33节点','接入18、33节点','接入8、15、18、33节点')
figure(3)
legend('接入33节点','接入18、33节点','接入8、15、18、33节点')