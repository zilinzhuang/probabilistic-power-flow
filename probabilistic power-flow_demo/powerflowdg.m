...
firstbranchbus = [0; firstbranchbus]; % 添加一条虚拟支路连接, 这样支路数就等于节点数，便用于后面迭代计算
Zbranch = [0; Zbranch];    %根据辐射网络的特征（支路数+1=节点）假设与首节点连接的支路阻抗为0，并把支路数加1.
NUMLINE = NUMLINE + 1;                    
...
% PI型 DG
PINUM = [2,7,12,15,23,28,32];    % PI型DG位置、光伏（电流型逆变器）   2,7,12,15,23,28     
Ppi_G = [1900/SB,1800/SB,1800/SB,1800/SB,1800/SB,1800/SB,1800/SB]; % kW >>> p.u.
...
% PQ(V)型 DG
PQVNUM = [31];    % PQV型DG位置  定速型异步风机（转差率不变，导致电压不变、无功吸收不变）
Ppqv_G = [30/SB]; % kW >>> p.u.
s_pqv=0.3;       % 转差率
...
%% PI节点处理        
	...
	if  ~isempty(PINUM)                % 存在PI节点
      ...
      %% PV节点处理
    % 生成PV节点的等效阻抗矩阵 为了求解每增加单位电流下，该节点电压增量是多少。
...
figure(4)
 plot(imag(Slossppf),'-g<')
 hold on
 plot(real(Slossppf),'-k<')
 title('无功线损和有功线损')
 xlabel('节点编号')
 ylabel('功率(kW)')
 grid on
