...
figure(3)
h1 = histogram(plossppf,200,'Normalization','probability','EdgeColor','b');
hold on
title('有功网损分布图');xlabel('Ploss(kW)');ylabel('概率');

figure(4)
plot(Ploss,'LineWidth',3.0')
 title('各支路线损情况')
 xlabel('支路编号')
 ylabel('支路线损(kW)')
 grid on
...
plineloss=zeros(NUMLINE-1,NMC);         %线损
%RANDOMMC = SUIJIMCF(printnumbus,NMC); %随机抽样MC
%LHSMCMC =  LHSMC(printnumbus,NMC);    %LHSMC
time0 = clock;  % 计时开始
...
figure(4)
plot((mean(plineloss,2)),'LineWidth',3.0')
 title('各支路线损情况')
 xlabel('支路编号')
 ylabel('支路线损(kW)')
 grid on
...
windLHS = windLHS(Index1);      % 随机排列后的矩阵
sunLHS = sunLHS(Index2);      % 随机排列后的矩阵
pw1=wind2power(windLHS,9,1)*1000;       %  风能有功功率
ps1=solar2power(sunLHS,1000)*1000;       %  太阳能功率
...
Ipi_G = -30/IB; % A >>> p.u
% PQ(V)型 DG
PQVNUM = [17,33];    % PQV型DG位置
Ppqv_G = 800/SB; % kW >>> p.u.
Qpqv_G = Ppqv_G*0.05; % A >>> p.u
% PV型 DG         光伏、燃料电池
PVNUM = []; % PV型DG位置
...
figure(3)
h1 = histogram(plossppf,200,'Normalization','probability','EdgeColor','g');
hold on
title('有功网损分布图');xlabel('Ploss(kW)');ylabel('概率');
...
figure(3)
h1 = histogram(plossppf,200,'Normalization','probability','EdgeColor','r');
hold on
title('有功网损分布图');xlabel('Ploss(kW)');ylabel('概率');
...
figure(1)
legend('不加入分布式电源','PV型DG','PQ型DG','PQV型DG','PI型DG')
figure(2)
legend('不加入分布式电源','PV型DG','PQ型DG','PQV型DG')
figure(3)
legend('不加入分布式电源','PV型DG','PQ型DG','PQV型DG','PI型DG')
figure(4)
legend('不加入分布式电源','PV型DG','PQ型DG','PQV型DG','PI型DG')