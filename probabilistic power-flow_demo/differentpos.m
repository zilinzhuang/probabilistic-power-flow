%% 分布式电源在不同节点，电压情况、越限数、各支路线损情况


dgcons=1600+1j*500  %kW Kvar
dgposition=[0,3,8,15,18,22,25,33];
dgcons=dgcons./10000;    %标幺值
d=zeros(1,8);
...
if(dgposition(ibus) ~= 0)

Pdg=real(dgcons);
Qdg=imag(dgcons);
Bus(dgposition(ibus),2)=Bus(dgposition(ibus),2)-Pdg;
Bus(dgposition(ibus),3)=Bus(dgposition(ibus),3)-Qdg;
...
figure(1)
legend('不接DG','接入3节点','接入8节点','接入15节点','接入18节点','接入22节点','接入25节点','接入33节点')
figure(2)
legend('不接DG','接入3节点','接入8节点','接入15节点','接入18节点','接入22节点','接入25节点','接入33节点')
