function LHSMCMC = LHSMC(printnumbus,nn)
...
%% LHS
windLHS=wblinv([0 : 1/nn : 1],7,2);   %LHS抽样开始！
sunLHS=betainv([0 : 1/nn : 1],4,0.1);
Index1 = randperm(nn);   % 生成随机排列数1
Index2 = randperm(nn);   % 生成随机排列数2
windLHS = windLHS(Index1);      % 随机排列后的矩阵
sunLHS = sunLHS(Index2);      % 随机排列后的矩阵
pw1=wind2power(windLHS,5,1)*1000;       %  风能有功功率
ps1=solar2power(sunLHS,1000)*1000;       %  太阳能功率

%% 随机抽样MC
%%wind1=wblrnd(7,2,1,nn);                %  设定WEIBULL参数！scale parameter A and shape parameter B.后两项为矩阵形式
%%sun1=betarnd(4,0.1,1,nn);             %  设定BETA参数！
%%pw1=wind2power(wind1,5,1)*1000;       %  风能有功功率
%%ps1=solar2power(sun1,1000)*1000;       %  太阳能功率





Uprintbus=zeros(nn,NUMBUS);          %建立所有模拟的数据库矩阵
...
% PQ型 DG        风机
PQNUM = [33]; % PQ型DG位置
Ppq_G = pw1(1,mci)/SB; % kW >>> p.u.
Qpq_G = 350*tan(acos(0.9))/SB; % kVar >>> p.u  如何更精确的计算呢？？and 风机分类
% PI型 DG
PINUM = [];    % PI型DG位置
Ppi_G = 20/SB; % kW >>> p.u.
Ipi_G = -10/IB; % A >>> p.u
...
while iter_num < iter_max             %迭代次数
	...
	if max(dU) <= ep    % 若达到收敛精度
		Uppf=Uppf+(1/nn).*U;                     %求MC的期望值
        Uprintbus(mci,:)=U(:,1);  %求所有节点每次的模拟值。
        for t=1:NUMBUS
             if (abs(U(t))<0.95)||abs((U(t))>1.05)    %电压越限
...
LHSMCMC.UMC=Uprintbus;
LHSMCMC.UPPF=Uppf;
LHSMCMC.BIANYI = std(abs(Uprintbus))./mean(abs(Uprintbus)); 
end
