...
clear
close all
%% 能处理环网的不确定性潮流计算（目前能处理多环）

%% 假设条件

MODEL = data33; % 导入33节点数据。负荷、支路阻抗数据均为标幺值
...
loopnum=0;                                     % 环路数
if size(firstbranchbus,1) >NUMLINE              % 判断是否为弱环网。
loopnum=size(MODEL.branch,1)-NUMLINE;
firstbranchbus = MODEL.branch(1:NUMLINE,2); % 首节点列向量
Zbranch = MODEL.branch(1:NUMLINE,4)+1i*MODEL.branch(1:NUMLINE,5); % 支路阻抗矩阵p.u.
Ssloop = zeros(loopnum,1);    % 环路功率
loopbranch = MODEL.branch(NUMLINE+1:size(MODEL.branch,1),:) ;       % 环路数据
loop1 = loopbranch(:,1)-NUMLINE;                                 % 提取支路编号
end

if ~isempty(loop1)              % 求环网阻抗矩阵
    for s=1:loopnum
        for c=1:loopnum
            Xloop(s,c)=MODEL.Xloop(loop1(s),loop1(c));
            Rloop(s,c)=MODEL.Rloop(loop1(s),loop1(c));
        end
    end
    Zloop=[Xloop,Rloop;(-1).*Rloop,Xloop];
...
n=1;
m=1;
TempBranch=MODEL.branch(1:NUMLINE-1,:);  %目前可利用的支路（去除已经存的支路）
...
if loopnum>0           % 判断是否为弱环网。
Ssloop = zeros(loopnum*2,1);    % 环路功率矩阵清零，上面一半是无功，下面一半为有功。
Vloop = zeros(loopnum*2,1);     % 环路电压差与相位矩阵
Vloopold = zeros(loopnum*2,1);  % 前一次矩阵值
end
...
Loadkplf=normrnd(1,0.1,NUMBUS,1); %生成33*1的正态分布的随机数矩阵。
...
%% PI节点处理        
	...
	if max(dU) <= ep    % 若达到收敛精度
       ...
       if loopnum>0    % 若存在环路
        for s=1:loopnum
        firstnum = loopbranch(s,2);  %环路首节点
        tailnum =  loopbranch(s,3);  %环路尾节点
    
        Vloop(s) = real(U(firstnum)-U(tailnum));              %环网电压降落
        Vloop(s+loopnum) = imag(U(firstnum)-U(tailnum));     
        
        %loadbus(firstnum) = loadbus(firstnum)+p_compen + 1i*q_compen; %将功率补偿到节点负荷上。
        
        end

        Ssloop=inv(Zloop)*Vloop;

        for s=1:loopnum
        firstnum = loopbranch(s,2);  %环路首节点
        tailnum =  loopbranch(s,3);  %环路尾节点
    
        p_compen = real(Ssloop(s+loopnum));              %环网电压降落
        q_compen = imag(Ssloop(s));     
        ...
        loadbus(firstnum) = loadbus(firstnum)+p_compen + 1i*q_compen; %将功率补偿到节点负荷上。
            ...
            if max(abs(Vloop-Vloopold)) <= ep2             %开环点电压差变化量若收敛
