...
firstbranchbus = [0; firstbranchbus]; % 添加一条支路至所有支路的顶层, 这样支路数就等于节点数，便用于后面迭代计算
...
NMC=20000;                          %  monte-carlo模拟数

%% LHS
windLHS=wblinv([0 : 1/NMC : 1],7,2);      %LHS抽样开始！
sunLHS=betainv([0 : 1/NMC : 1],4,0.1);
LoadLHS=norminv([0.000001 : (1-0.000001)/NMC : 1],1,0.1);   %负荷近似正态分布概率曲线,其概率累计分布不好取零，否则为负无穷。
% 负荷变化概率

Loadkplf=ones(NUMBUS,NMC);                % 负荷样本保存的矩阵

for iload=1:NUMBUS
Index1 = randperm(NMC);
LoadLHS = LoadLHS(Index1);
Loadkplf(iload,:) = LoadLHS;

end 

Index1 = randperm(NMC);   % 生成随机排列数1
Index2 = randperm(NMC);   % 生成随机排列数2
Index3 = randperm(NMC);   % 生成随机排列数2
Index4 = randperm(NMC);   % 生成随机排列数2
Index5 = randperm(NMC);   % 生成随机排列数2
Index6 = randperm(NMC);   % 生成随机排列数2
Index7 = randperm(NMC);   % 生成随机排列数2
Index8 = randperm(NMC);   % 生成随机排列数2

windLHS = windLHS(Index1);      % 随机排列后的矩阵
sunLHS1 = sunLHS(Index2);      % 随机排列后的矩阵
sunLHS2 = sunLHS(Index3);      % 随机排列后的矩阵
sunLHS3 = sunLHS(Index4);      % 随机排列后的矩阵
sunLHS4 = sunLHS(Index5);      % 随机排列后的矩阵
sunLHS5 = sunLHS(Index6);      % 随机排列后的矩阵
sunLHS6 = sunLHS(Index7);      % 随机排列后的矩阵
sunLHS7 = sunLHS(Index8);      % 随机排列后的矩阵

pw1=wind2power(windLHS,3,1)*1000;       %  风能有功功率
ps1=solar2power(sunLHS1,pp)*1000;       %  太阳能功率
ps2=solar2power(sunLHS2,pp)*1000;       %  太阳能功率
ps3=solar2power(sunLHS3,pp)*1000;       %  太阳能功率
ps4=solar2power(sunLHS4,pp)*1000;       %  太阳能功率
ps5=solar2power(sunLHS5,pp)*1000;       %  太阳能功率
ps6=solar2power(sunLHS6,pp)*1000;       %  太阳能功率
ps7=solar2power(sunLHS7,pp)*1000;       %  太阳能功率
...
% PI型 DG
PINUM = [2,7,12,15,23,28,32];    % PI型DG位置、光伏（电流型逆变器）2,7,12,15,23,28,32        
Ppi_G = [ps1(1,mci)/SB,ps2(1,mci)/SB,ps3(1,mci)/SB,ps4(1,mci)/SB,ps5(1,mci)/SB,ps6(1,mci)/SB,ps7(1,mci)/SB]; % kW >>> p.u.
Ipi_G = [-10/IB,-10/IB,-10/IB,-10/IB,-10/IB,-10/IB,-10/IB]; % A >>> p.u
Q_PI=zeros(1,length(PINUM));
Q_OLDPI=zeros(1,length(PINUM));


% PQ(V)型 DG
PQVNUM = [21];    % PQV型DG位置  定速型异步风机（转差率不变，导致电压不变、无功吸收不变）
Ppqv_G = [30/SB]; % kW >>> p.u.
s_pqv=0.3;       % 转差率
...
loadbus(:,1)=Sd_orginal(:,1).*Loadkplf(:,mci);
...
ucrossline7(:,(pp/10)+1)=Ucrossline/NMC;
end
